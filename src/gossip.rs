...
impl MessageCache {
    ...
    fn evict_old_messages(&mut self) {
        ...
        self.known_messages.retain(|_, timestamp| now - *timestamp < 3600);
        ...
        if self.known_messages.len() >= self.max_size {
            let entries_to_remove: Vec<u64> = {
                ...
                entries.iter().take(remove_count).map(|(message_id, _)| **message_id).collect()
            };
    ...
    fn evict_old_messages_from_cache(&mut self) {
        ...
        if self.message_cache.len() > self.max_size {
            let hashes_to_remove: Vec<u64> = {
                ...
                let remove_count = self.max_size / 4;
                entries.iter().take(remove_count).map(|(hash, _)| **hash).collect()
            };
...
impl GossipProtocol {
    ...
    pub async fn handle_message(&self, envelope: MessageEnvelope) -> Result<(), SynapseError> {
        ...
        match &envelope.message {
            ...
            GossipMessage::SyncRequest { known_hashes } => {
                ...
                self.handle_sync_request(&envelope.source_node, known_hashes).await?;
            }
            
            GossipMessage::SyncResponse { missing_messages } => {
                debug!("Received sync response with {} messages", missing_messages.len());
    ...
    async fn forward_message(&self, envelope: MessageEnvelope) -> Result<(), SynapseError> {
        ...
        use rand::seq::SliceRandom;
        let mut rng = rand::thread_rng();
        let selected_peers: Vec<_> = forward_peers
    ...
    async fn start_sync_timer(&self) -> Result<(), SynapseError> {
        ...
        loop {
            ...
            let sync_request = MessageEnvelope::new(
                node_id.clone(),
                GossipMessage::SyncRequest { known_hashes },
            );
            ...
            let peers_guard = peers.read().await;
            let healthy_peers: Vec<_> = peers_guard
                .values()
    ...
    async fn start_message_processor(&self) -> Result<(), SynapseError> {
        let mut inbound_rx = {
            ...
            rx_guard.take().ok_or_else(|| {
                SynapseError::config("Message processor already started")
            })?
        };
    ...
    async fn handle_sync_request(&self, requester: &str, known_hashes: &[u64]) -> Result<(), SynapseError> {
...
mod tests {
    ...
    async fn test_gossip_protocol_creation() {
        ...
        let (protocol, _outbound_rx) = GossipProtocol::new(
            "test-node".to_string(),
            config,
            message_tx,
        );
