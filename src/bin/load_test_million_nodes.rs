...
impl LightweightNode {
    fn new(id: usize, base_port: u16) -> Self {
        Self {
            ...
            port: base_port.saturating_add((id as u16) % 10000), // Safer port calculation
...
impl MillionNodeLoadTest {
    ...
    pub async fn run_clustered_topology_test(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "🏗️  Creating {} clusters with {} nodes each",
        ...
        self.test_inter_cluster_messaging(&cluster_representatives)
            .await?;

        // Sustained load test
        println!(
            "⚡ Running sustained load test for {} seconds...",
    ...
    async fn create_cluster(
    ...
    ) -> Result<ClusterStats, Box<dyn std::error::Error>> {
        ...
        for node in lightweight_nodes.iter() {
            let connection_count = std::cmp::min(5, lightweight_nodes.len() - 1);
            node.connection_count
                .store(connection_count, Ordering::Relaxed);
            self.stats
                .connections_established
                .fetch_add(connection_count, Ordering::Relaxed);
        }
    ...
    async fn test_inter_cluster_messaging(
        &self,
        ...
        println!(
            "🧪 Creating {} real SynapseNodes for messaging test",
            test_nodes_count
        );
        ...
        for i in 0..test_nodes_count {
            ...
            let handle = tokio::spawn(async move {
                ...
                if i > 0 {
                    ...
                    if (node.add_peer(peer).await).is_err() {
        ...
        println!(
            "✅ Inter-cluster test: {}/{} nodes completed successfully",
            completed_nodes, test_nodes_count
        );
        Ok(())
    }
        ...
        let monitor_handle = tokio::spawn({
            ...
            async move {
                while start_time.elapsed() < duration {
                    ...
                    println!(
                        "📊 Sustained load: {} messages sent, {:.1} msg/sec",
                        messages, rate
                    );
                }
            }
    ...
    fn print_stats(&self) {
        ...
        println!(
            "📊 Stats: {} nodes, {} connections, {} messages, {} errors",
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Default million node load test
    let config = LoadTestConfig::default();
    let load_test = MillionNodeLoadTest::new(config);
    load_test.run_clustered_topology_test().await
}

// Test configurations for different scales
#[cfg(all(test, feature = "load-tests"))]
mod tests {
    use super::*;
    ...
    async fn test_million_node_load_test() {
        ...
        assert!(
            result.is_ok(),
            "Million node load test should complete without panicking"
        );
    }
}
